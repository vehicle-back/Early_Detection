...
def lgbm_step_inc_train_eval(train_X, train_Y, val_X, val_Y, test_X, test_Y, i, args):
    ...
    if not args.use_laststep_pred:
        ...
        lgb_train = lgb.Dataset(train_X, train_Y[:, i, 3])
        lgb_eval = lgb.Dataset(val_X, val_Y[:, i, 3], reference=lgb_train)
        ...
        test_pred = model.predict(test_X, num_iteration=model.best_iteration) 
        print('*****************************************************************')
        val_ground_truth = val_Y[:, i, 3]
        test_ground_truth = test_Y[:, i, 3]
...
def gbm_create_parser():
    ...
    parser.add_argument('--county', type=str, default="Cranberry" #"TSMO"
                        , help="Choose one from 'Cranberry', 'TSMO'")
    parser.add_argument('--link_id', type=str, default = '104+04542' #'110N04483' #'110+04484' #'110-04482' #'110+04483' # '110P04483' #
    ...
    parser.add_argument('--number_of_business_day', type=int, default = 522
                       # 260
    ...
    '''
    parser.add_argument('--task', type=str, default = 'LR', help="Choose one from 'LR', 'rec', 'nonrec', 'finetune', 'no_fact', 'naive', 'naive_2enc") 
